...
| スキル | 説明 | 用途 |
|---|---|---|
| `/survey` | 大量ペルソナによるアンケート調査 | 定量データ収集（100人規模のリッカート尺度・選択式・自由記述） |
| `/ux-test` | ペルソナ視点でChromeを実操作 | ユーザビリティ評価（画面遷移・操作感・つまずきポイント） |
| `/interview` | 1人のペルソナと対話形式でインタビュー | 定性データ収集（課題の深掘り・動機の理解） |

### 推奨フロー

```
/survey → /interview → /ux-test
```

1. `/survey` で定量的に全体傾向を把握する
2. `/interview` で注目テーマを深掘りする
3. `/ux-test` で実画面の操作性を検証する
...
| エージェント | モデル | 説明 |
|---|---|---|
| `survey-respondent` | haiku | アンケート回答を大量生成する（`/survey` から呼び出し） |
| `interviewee` | sonnet | ペルソナとして対話に応じる（`/interview` から呼び出し） |
